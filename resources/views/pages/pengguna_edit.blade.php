        ...
        <div class="flex sm:gap-2 flex-wrap sm:flex-nowrap">
            ...
            <div class="sm:w-9/12 w-full">
                <div class="px-5 border rounded-lg">
                    <div class="my-2">
                        <h1 class="text-2xl text-slate-700 font-semibold">Edit Pengguna</h1>
                    </div>
                    ...
                    <div class="py-5">
                        ...
                        @if (session()->has('status'))
                            ...
                            @endif
                        @endif
                        <form action="{{ route('pengguna.update',$pengguna->id) }}" method="post" autocomplete="off">
                            @csrf
                            @method('PUT')
                            <x-input-text name="name" label="Nama Pengguna" value="{{ $pengguna->name }}" />
                            <x-input-text name="email" label="Email Pengguna" type="email" value="{{ $pengguna->email }}" />
                            <x-input-text name="password" label="Password Pengguna" type="password" placeholder="Kosongkan jika tidak diganti" required="{{ false }}" />
